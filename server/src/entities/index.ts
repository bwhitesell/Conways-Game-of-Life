...
async function syncDBConnectedModels() {
  ...
  if (ENV === "development") {
    console.log(
      "Environment detected as development. Syncing sequelize models..."
    );
    // clear out redis if running in dev
    if (CLEAR_REDIS_ON_DEV_START) {
      new Redis(REDIS_URI).flushall("ASYNC", () =>
    ...
    console.log(
      "Environment not detected as development. Sequelize models not being auto-syncd."
    );
  }
}
